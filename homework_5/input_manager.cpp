...
namespace InputManager {
  ...
  constexpr uint16_t joyLowerDirThreshold = 300;
  constexpr uint16_t joyHigherDirThreshold = 700;
  constexpr uint16_t joyMinNeutralValue = 400;   // The joystick potentiometer reading for a given position can also return close values.  
  constexpr uint16_t joyMaxNeutralValue = 600;   // Solves the problem when the joystick is at the exact threshold between neutral and direction.
  ...
  void getButtonState() {
    ...
    if (millis() - lastSWChangeTime >= debounceDelay) {
      ...
      // if (currentJoySWState == LOW)
      //   buttonPressDuration = millis() - lastSWChangeTime;
    }
  }
